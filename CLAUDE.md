...
1. Political bias/leaniancy of articles.
    ...
    Implemented so far (a fixed demo only; not wired into ordinary searches, and per-source bias is not done):
    - **`GET /api/leaning-demo`** - a fixed, small demonstration: the Guardian's latest Politics articles (`leaning.demo-count`, default 5; `leaning.demo-sections`, default `politics`), each read **in full** and assessed by **Claude** (`PoliticalLeaningService`, via the official `anthropic-java` SDK - `anthropic.model`, default `claude-sonnet-5`; a general-purpose LLM, not a specialised classifier). Returns the usual `{ articles, warnings }` envelope, `leaning` set on each article. This is the only thing in the app that calls Claude - an ordinary `/api/articles` search, including one filtered to the Politics section, is never classified, so it costs nothing extra and isn't slowed down. The old `POST /api/leaning` (single-text endpoint) and the local `matous-volf/political-leaning-politics` classifier (Python sidecar in `ml/`) are gone.
    - **Output shape**: `Leaning(label, score, explanation, excerpts)` - `label` LEFT/CENTER/RIGHT; `score` is **Claude's own stated confidence (0-1), not a calibrated statistic** the way a trained classifier's probability would be; `explanation` a short paragraph grounded in the article's framing, word choice, sourcing or omissions; `excerpts` 0-5 `Excerpt(quote, side, reason)` - exact quotes from the article. Produced via the SDK's **structured output** feature (`StructuredMessageCreateParams<LlmAssessment>` + a Java record with `@JsonPropertyDescription`s that double as the prompt instructions - the SDK derives the JSON schema and parses the reply, no manual JSON parsing).
    - **Excerpt verification**: every quote Claude returns is checked as an actual substring of the article text it was given (case-insensitive, whitespace-tolerant) before being shown; a quote that doesn't verify is dropped rather than shown as a fabricated excerpt attributed to the article. `label`/`score`/`explanation` are kept regardless.
    - **Full article body**: the Guardian's `body` field (`GuardianProvider.SHOW_FIELDS`) is fetched on every Guardian search and kept on `Article.fullText`, `@JsonIgnore`d - it is read server-side for classification only and never reaches the browser (confirmed: absent from `/api/articles` and `/api/leaning-demo` JSON). No truncation - a Guardian article is a few thousand tokens at most, trivial against Sonnet 5's 1M-token context.
    - **Cost stays bounded** without a daily spend cap: `GuardianProvider.search` and `PoliticalLeaningService.classify` (keyed per article URL) are both `@Cacheable` (1h TTL), so a warm reload of the demo panel costs zero Guardian or Claude calls - worst case is `leaning.demo-count` Claude calls per cache window, not per page load.
    - **Setup**: `AnthropicApiKey` in `.env` (from console.anthropic.com); `anthropic.timeout` (default 45s) bounds one hung call. Without a key the rest of the app still works - the demo panel just shows nothing plus a warning.
    - **Caveats**: this is a demonstration of the mechanism, not a validated bias measure - spot checks showed confident-looking labels on UK political stories the (originally US-trained-classifier) framing doesn't transfer well to. Claude's confidence score is its own self-assessment, not evidence of accuracy.
